...
uint256 hashGenesisBlock("0x78b2faa7e24807b1e90d2368ef253cac51a45b9ee9e8f2e325a77a95c7d80c4f");
...
const string strMessageMagic = "ruthenium Signed Message:\n";
...
{
    // ruthenium: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // ruthenium: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "ruth genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	    ...
	    //assert(block.hashMerkleRoot == uint256("0x95e6a0e897ed76cd5f08b75d118fb7c99aec7cdd297b96c21dc6671d2034c953"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd5, 0xc1, 0xaf, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("rutheniumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("rutheniumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static rutheniumMiner(CWallet *pwallet)
{
    printf("rutheniumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ruthenium-miner");
    ...
    try { loop {
        ...
        printf("Running rutheniumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("rutheniumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&rutheniumMiner, pwallet));
}
