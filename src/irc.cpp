...
// Copyright (c) 2011-2012 ruthenium Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("ruthenium-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #rutheniumTEST0\r");
            Send(hSocket, "WHO #rutheniumTEST0\r");
        } else {
            // randomly join #ruthenium00-#ruthenium99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #ruthenium%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #ruthenium%02d\r", channel_number).c_str());
        }
